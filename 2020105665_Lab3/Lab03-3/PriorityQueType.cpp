...
bool PriorityQueType::IsEmpty(int priority)
{
    ...
    if(priority==1){
        result = (high_front == high_rear)?true:false;
    }else if(priority==2){
        result = (low_front == low_rear)?true:false;
    }
...
bool PriorityQueType::IsFull(int priority)
{
    ...
    if(priority==1){
        result = ((high_rear + 1) % maxQue == high_front)?true:false;
    }else if(priority==2){
        result = ((low_rear + 1) % maxQue == low_front)?true:false;
    }
...
ItemType PriorityQueType::Dequeue()
{
    ...
    if(!IsEmpty(HIGH)){
        if(high_front==(maxQue-1)){
            item = high_items[0];
            high_front = 0;
        }else{
            item = high_items[high_front+1];
            high_front++;
        }
    }else{
        if(low_front==(maxQue-1)){
            item = low_items[0];
            low_front = 0;
        }else{
            item = low_items[low_front+1];
            low_front++;
        }
    }
